        ...
        match result {
            Outcome::Continue(updates) => {
                for update in updates.into_iter().flatten() {
                    match update {
                        Update::Fade(inout) => if !animation.contains_key(&0) {animation.insert(0u8, if inout {Box::new(FADE_IN.iter().cloned())} else {Box::new(FADE_OUT.iter().cloned())});}, 
                        _ => ()
                    }
                }
                display.draw_room(&play, &mut animation, &context.sprites, &backdrop)
            },
            Outcome::Dead => play.reset(match target {Entrance::Flying(side, ..) => Entrance::Spawn(side), target => target}),
